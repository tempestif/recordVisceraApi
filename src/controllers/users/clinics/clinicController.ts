...
import { createFilterForPrisma, createSortsForPrisma, filteringFields } from "@/services/dataTransferService";
...
export const registClinicReport = async (req: Request, res: Response, next: NextFunction) => {
    // logのために関数名を取得
    const currentFuncName = registClinicReport.name

    // bodyから情報取得
    const { userId, date, checkupBlood, checkupMri, checkupCt, checkupCustom } = req.body
    // TODO: バリデーション
    if (typeof checkupBlood !== "boolean" || typeof checkupMri !== "boolean" || typeof checkupCt !== "boolean") {
        throw Error
    }
    ...
    try {
        ...
        basicHttpResponceIncludeData(res, HttpStatus, responseStatus, responseMsg, clinicReport)
 ...
 * @param next
 */
export const readClinicReport = async (req: Request, res: Response, next: NextFunction) => {
    // logのために関数名を取得
